...
The method inserts a new order into the warehouse system. It also updates an existing order when a request with an
already existing `original_order_id` (matched together with `shop_setting_id` and `order_type`) is sent. An order can be
updated only while its current status allows it - i.e. while the status has `api_update_enabled` = `true` (the same
`api_update_enabled` flag is returned by the [GetOrderStatus](GetOrderStatus.md) method). There must be at least
**20 seconds** between two update requests for the same order.
...
| `shop_setting_id`                  |                  | (Integer) | [link](https://egon.isklad.com/klient/settings-shop-settings) |        default: 1         | Set-to-order setting ID. :warning: if omitted, the first shop setting (ID 1) is used - it is strongly recommended to always send the correct `shop_setting_id`                                                                                                                                                                                                                                                                            |
...
| `myorder_identifier`               |                  | (String)  |                               -                               |           empty           | Reference Nr. of myorder virtual order                                                                                                                                                                                                                                                             |
...
| `phone`                            |                  | (String)  |                               -                               |           empty           | Delivery - Phone                                                                                                                                                                                                                                                                                   |
...
| `declared_value`                   |                  | (Decimal) |                               -                               |           empty           | Declared value of the order                                                                                                                                                                                                                                                                        |
...
| `insurance_price`                  |                  | (Decimal) |                               -                               |           empty           | Price for insurance including VAT                                                                                                                                                                                                                                                                  |
| `coupon_discount_amount`           |                  | (Decimal) |                               -                               |           empty           | Coupon - value of applied amount to order                                                                                                                                                                                                                                                          |
...
| `forced_completion`                |                  | (Boolean) |                               -                               |           false           | If something is missing from the order, the order will automatically be sent without the missing piece and another order will be created for the missing piece.                                                                                                                                    |
| `is_bank_account_needed_for_claim` |                  | (Boolean) |                               -                               |           empty           | If order has a fixed flag, when needed or not needed to ask customer for bank account at the claim process in MyOrder order claim process (default is empty = determine from settings in the client system)                                                                                        |
