...
| `is_bank_account_needed_for_claim` |                  | (Boolean) |                               -                               |           empty           | If order has a fixed flag, when needed o not needed to ask customer for bank account at the claim process in MyOrder order claim process (default is empty = determine from settings in the client system)                                                                                         |
