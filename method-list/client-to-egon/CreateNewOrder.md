...
| `order_type`                 |                  | (String)  |  fulfillment / shipping_label / virtual_order / return_order  |   default: fulfillment    | Order type (`fulfillment` - if the whole fulfillment process is managed by EGON, `shipping_label` - if the fulfillment process is managed by an external company, or client himself, `virtual_order` - for notification purposes only, `return_order` - for return orders)                         |
